...
function PostContent({ id }: { id: string }) {
  ...
  const post = useQuery(api.forums.getPostById, {
    postId: id as Id<"forumPosts">,
  });
  const replies = useQuery(api.forums.getReplies, {
    postId: id as Id<"forumPosts">,
  });
  const hasLiked = useQuery(
    api.forums.hasLikedPost,
    currentUser ? { postId: id as Id<"forumPosts">, userId: currentUser._id } : "skip"
  );
  ...
  const handleLikePost = async () => {
    ...
    await togglePostLike({ postId: id as Id<"forumPosts"> });
  };
  ...
  const handleReply = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!currentUser || !replyContent.trim() || isSubmitting) return;
  ...
  const handleDeletePost = async () => {
    ...
    await deletePost({ postId: id as Id<"forumPosts">, userId: currentUser._id });
  ...
  const handleTogglePin = async () => {
    if (!currentUser) return;
    await togglePin({ postId: id as Id<"forumPosts">, userId: currentUser._id });
  };
  ...
  const getCategoryColor = (cat: string) => {
    ...
    return colors[cat] || "bg-gray-100 text-gray-700 dark:bg-muted dark:text-muted-foreground";
  };
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <main className="max-w-[900px] mx-auto px-4 md:px-6 py-6 md:py-8">
          ...
          <div className="flex items-start gap-4 mb-4">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-sm text-muted-foreground">
                {post.author?.name || "Unknown"} &middot; {getTimeAgo(post.createdAt)}
              </p>
            </div>
          </div>
        ...
        <section className="mb-6">
          ...
          {replies.length > 0 ? (
            <div className="space-y-3">
              {replies.map((reply) => {
                const isReplyAuthor = currentUser && reply.authorId === currentUser._id;
                return (
                  <div
                    key={reply._id}
                    ...
                    <div className="flex items-start gap-3">
                      <div className="shrink-0">
                        {reply.author?.imageUrl ? (
                          <div
                            ...
                            style={{ backgroundImage: `url(${reply.author.imageUrl})` }}
                          />
                        ) : (
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 mb-1">
                          <span className="text-sm font-bold">{reply.author?.name || "Unknown"}</span>
        ...
        {isAuthenticated && (
          <form onSubmit={handleReply} className="bg-white dark:bg-card rounded-xl border border-gray-100 dark:border-border p-4">
            <div className="flex items-start gap-3">
