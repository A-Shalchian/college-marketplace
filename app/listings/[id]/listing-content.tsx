...
const campusMapUrls: Record<string, string> = {
  "St. James Campus":
    "https://maps.google.com/maps?q=George+Brown+College+St+James+Campus,Toronto&z=15&output=embed",
  "Casa Loma Campus":
    "https://maps.google.com/maps?q=George+Brown+College+Casa+Loma+Campus,Toronto&z=15&output=embed",
  "Waterfront Campus":
    "https://maps.google.com/maps?q=George+Brown+College+Waterfront+Campus,Toronto&z=15&output=embed",
};

export function ListingContent({
  preloadedListing,
}: {
  preloadedListing: Preloaded<typeof api.listings.getById>;
}) {
  const router = useRouter();
  const { isAuthenticated } = useConvexAuth();
  const [selectedImageIndex, setSelectedImageIndex] = useState(0);

  const listing = usePreloadedQuery(preloadedListing);
  const currentUser = useQuery(api.users.getCurrentUser);
  const existingConversation = useQuery(
    api.messages.getExistingConversation,
    currentUser && listing
      ? { listingId: listing._id, buyerId: currentUser._id }
      : "skip"
  );
  ...
  const handleContactSeller = () => {
    ...
    if (existingConversation) {
      ...
      router.push(
        `/messages/new?listing=${listing._id}&seller=${listing.seller._id}`
      );
    }
  };
                  ...
                  <div className="mt-3 flex gap-2 items-start">
                    ...
                    <div className="text-sm">
                      <p className="font-medium">
                        {listing.campus || "GBC Campus"}
                      </p>
                      <p className="text-gray-500">
                        Available for meetup at Student Centre
                      </p>
                    </div>
                  </div>
            ...
            <div className="lg:sticky lg:top-24 space-y-4 md:space-y-6">
                ...
                <div className="flex items-center gap-4 mb-4">
                  {listing.seller?.imageUrl ? (
                    <div
                      ...
                      style={{
                        backgroundImage: `url(${listing.seller.imageUrl})`,
                      }}
                    />
                  ) : (
              ...
              <div className="p-4 md:p-5 rounded-xl bg-primary/5 border border-primary/20 flex gap-4">
                ...
                <div className="text-xs leading-relaxed text-primary">
                  ...
                  Always meet in well-lit, public campus areas like the Student
